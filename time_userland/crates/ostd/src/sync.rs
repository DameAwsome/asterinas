...
impl<T> Mutex<T> {
    pub const fn new(val: T) -> Self {
        Self { locked: AtomicBool::new(false), val: UnsafeCell::new(val) }
    }
}
...
impl<T: ?Sized> Mutex<T> {
    pub fn lock(&self) -> MutexGuard<'_, T> {
        ...
        // SAFETY: lock acquired
        unsafe { MutexGuard { m: self } }
    }
}
...
impl<'a, T: ?Sized> Deref for MutexGuard<'a, T> {
    type Target = T;
    fn deref(&self) -> &Self::Target { unsafe { &*self.m.val.get() } }
}
impl<'a, T: ?Sized> DerefMut for MutexGuard<'a, T> {
    fn deref_mut(&mut self) -> &mut Self::Target { unsafe { &mut *self.m.val.get() } }
}
